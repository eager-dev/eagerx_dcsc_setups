...
def simulate(
    ...
    # Define engines
    engine_ode_sync = Engine.make("OdeEngine", rate=engine_rate, sync=True, real_time_factor=0, process=process.NEW_PROCESS)
    engine_ode_async = Engine.make("OdeEngine", rate=engine_rate, sync=False, real_time_factor=0, process=process.NEW_PROCESS)
    ...
    for name, params in envs.items():
        ...
        # Choose engine
        engine_ode = engine_ode_sync if params["sync"] else engine_ode_async

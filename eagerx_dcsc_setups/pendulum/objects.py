...
class Pendulum(Object):
    ...
    def agnostic(spec: ObjectSpec, rate):
        ...
        spec.sensors.pendulum_output.space_converter = SpaceConverter.make('Space_AngleDecomposition', low=[-1, -1, -9],
                                                                       high=[1, 1, 9], dtype='float32')

        spec.sensors.action_applied.rate = rate
        spec.sensors.action_applied.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-3], high=[3],
                                                                          dtype='float32')

        spec.sensors.image.rate = 15
        spec.sensors.image.space_converter = SpaceConverter.make('Space_Image', low=0, high=1,
                                                                 shape=spec.config.render_shape, dtype='float32')
        ...
        spec.actuators.pendulum_input.space_converter = SpaceConverter.make('Space_Float32MultiArray', low=[-3], high=[3],
                                                                        dtype='float32')

        # Set model_state properties: (space_converters)
        spec.states.model_state.space_converter = SpaceConverter.make('Space_Float32MultiArray',
                                                                      low=[-3.14159265359, -9], high=[3.14159265359, 9],
                                                                      dtype='float32')
    ...
    def spec(spec: ObjectSpec, name: str, sensors=None, states=None,
             rate=30, always_render=False, render_shape=None, camera_index=2):
    ...
    def ode_bridge(spec: ObjectSpec, graph: EngineGraph):
        ...
        image = EngineNode.make('PendulumImage', 'image', shape=spec.config.render_shape, rate=spec.sensors.image.rate,
                                process=0)

        # Create actuator engine nodes
        action = EngineNode.make('OdeInput', 'pendulum_actuator', rate=spec.actuators.pendulum_input.rate, process=2,
                                 default_action=[0])
    ...
    def real_bridge(spec: ObjectSpec, graph: EngineGraph):
        ...
        applied = EngineNode.make('ActionApplied', 'applied', rate=spec.sensors.action_applied.rate, process=0)
        image = EngineNode.make('CameraRender', 'image', camera_idx=spec.config.camera_index,
                                shape=spec.config.render_shape, rate=spec.sensors.image.rate, process=0)
